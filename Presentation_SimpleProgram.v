...
Module SimpleProgram.
   ...
   *)
   ...
   (* We must define the name of the relation, its variables,
    ...
    Let meaning := program_semantics_eval G'' program1 3.
    Eval compute in meaning.
    ...
    Eval compute in (find_meaning meaning "T").
    Let found := find_meaning meaning.

    Print found.
    Eval compute in found.

    (* Let find_meaning' (m: option gm_type) x := match m with
    | Some m' => option_map (the thing to apply func to) (functino to apply) *)

    (* This is a helper function to  *)
    Let find_meaning (m: option gm_type) x := match m with
    | Some m' => option_map (@MoreOrders.ground_maps.this (list (list ground_types))) (MoreOrders.ground_maps.find x m')
    | None => nil
    end.
